...
\begin{document}
\lecture{5}{}{Avery Li}{January 15, 2025}
...
\begin{proof}
    For every $d\vert n$, define $S_d=\{m\vert 1\leq m\leq n\text{ s.t. } (m,n)=d\}$.
    Then $\bigcup_{d\vert n}S_d=\{1\dots n\}$ and the set of $S_d$'s partition $\{1\dots n\}$.
    Note that $(m,n)=d\iff(m/d, n/d)=1\implies S_d=\{kd\vert 1\leq k\leq n/d \text{ s.t. } (k, n/d)=1\}$.
    By definition, $\lvert S_d\rvert=\varphi(n)$.
    Thus,
    \begin{align*}
        n&=\sum_{d\vert n}\lvert S_d\rvert \\
        &=\sum_{d\vert n}\varphi(n/d) \\
        n&=\sum_{d\vert n}\varphi(d) \\
    \end{align*}
\end{proof}

\section*{Convolution of Functions}
The convolution of functions defined over arithmetic functions is as follows:
\begin{definition}[Convolution of arithmetic functions]
    Let $f, g$ be arithmetic functions. The convolution of $f$ and $g$ is defined as
    \begin{align*}
        (f*g)(n)=\sum_{d\vert n}f(d)g\Big(\frac{n}{d}\Big)=\sum_{d\vert n}f\Big(\frac{n}{d}\Big)g(d)=(g*f)(n)
    \end{align*} 
\end{definition}
